...
{      
  ...
  // default at power-up:
  // Gyro at 250 degrees second, Acceleration at 2g, Clock source at internal 8MHz. The device is in sleep mode.
...
void loop()
{
  ...
  {
    //store in array
    accelx[count]=accel_t_gyro.value.x_accel;
    accely[count]=accel_t_gyro.value.y_accel;
    accelz[count]=accel_t_gyro.value.z_accel;
    gyrox[count]=accel_t_gyro.value.x_gyro;
    gyroy[count]=accel_t_gyro.value.y_gyro;
    gyroz[count]=accel_t_gyro.value.z_gyro;
    asumx+=accelx[count];
    asumy+=accely[count];
    asumz+=accelz[count];
    gsumx+=gyrox[count];
  ...
  count=0;
  //avg out 100 values
  accelx_avg=asumx/100;
  accely_avg=asumy/100;
  accelz_avg=asumz/100;
  
  gyrox_avg=gsumx/100;
  gyroy_avg=gsumy/100;
  gyroz_avg=gsumz/100;
  
  //convert accel to SI unit
  double tempax=accelx_avg*9.8;
  double tempay=accely_avg*9.8;
  double tempaz=accelz_avg*9.8;
  axSI=tempax/16384.0;
  aySI=tempay/16384.0;
  azSI=tempaz/16384.0;
  ...
  accXdis=(axSI*(dtime/1000))*(dtime/1000);
  accYdis=(aySI*(dtime/1000))*(dtime/1000);
  accZdis=(azSI*(dtime/1000))*(dtime/1000);
  
  //accel to angle
 
  R = sqrt(pow(axSI,2)+pow(aySI,2)+pow(azSI,2));
  accXangle = acos(axSI/R)*RAD_TO_DEG-90;
  accYangle = acos(aySI/R)*RAD_TO_DEG-90;
  
  //convert raw gyro to deg/sec
  gyroXrate=((double)gyrox_avg/131);
  gyroYrate=((double)gyroy_avg/131); 
  gyroZrate=((double)gyroz_avg/131);
  
  //calculate gyro angle 
  gyroXangle += gyroXrate*dtime/1000;
  gyroYangle += gyroYrate*dtime/1000;
  gyroZangle += gyroZrate*dtime/1000;
  
  //dtime = millis()-timer;
  //timer = millis();
  //add complementary filter to combine accel+gyro to one set
  ...
  //kalman filter
  kalAngleX = kalmanX.getAngle(accXangle, gyroXrate, (double)dtime/1000); 
  kalAngleY = kalmanY.getAngle(accYangle, gyroYrate, (double)dtime/1000);
  
  //timer = micros();
  //send to serial 
 
  //convert to (x,y)
  pointx=accXdis * cos (accXangle);
  pointy=(-1) * accYdis * sin (accYangle);
  
  Serial.print(pointx);Serial.print("\t");
  Serial.print(pointy);Serial.print("\t"); 
  
  //Serial.print(accXangle);Serial.print("\t");
  //Serial.print(accYangle);Serial.print("\t"); 
  
  //Serial.print(accXdis);Serial.print("\t");
  //Serial.print(accYdis); Serial.print("\t");
